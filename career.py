...
@router.message(Command(commands=['career']))
@router.message(F.text.lower() == 'карьера')
async def start(message: types.Message, state: FSMContext):
    await message.answer('Какая профессия вас интересует?',
                         reply_markup=keyboards.make_keyboard(available_jobs))
...
async def jobs(message: types.Message, state: FSMContext):
    await message.answer('Как вы оцениваете свою профессию?',
